...
namespace ssMath
{
        ...
        Matrix3& kR)
    {
        ...
        float fZ = kA[0][1];
		float fInvLength = ss_fast_InvSqrt(fY*fY+fZ*fZ)/*Math::InvSqrt(fY*fY+fZ*fZ)*/;
        ...
        fY = kA[0][0];
        fZ = kA[1][0];
        fInvLength = ss_fast_InvSqrt(fY*fY+fZ*fZ)/*Math::InvSqrt(fY*fY+fZ*fZ)*/;
        fSin = fZ*fInvLength;
        fCos = -fY*fInvLength;
        ...
        for (unsigned int i = 0; i < ms_iSvdMaxIterations; i++)
        {
            ...
            if ( bTest1 )
            {
                ...
                else
                {
                    ...
                    fCos0 = ss_fast_InvSqrt(1.0f+fTan0*fTan0)/*Math::InvSqrt(1.0f+fTan0*fTan0)*/;
                    fSin0 = fTan0*fCos0;
                    ...
                    fCos1 = ss_fast_InvSqrt(1.0f+fTan1*fTan1)/*Math::InvSqrt(1.0f+fTan1*fTan1)*/;
                    fSin1 = -fTan1*fCos1;
    ...
    void Matrix3::orthonormalize ()
    {
        ...
        // compute q0
        float fInvLength = ss_fast_InvSqrt(m[0][0]*m[0][0]//Math::InvSqrt(m[0][0]*m[0][0]
            + m[1][0]*m[1][0] +
            m[2][0]*m[2][0]);
        ...
        fInvLength = ss_fast_InvSqrt(m[0][1]*m[0][1] +//Math::InvSqrt(m[0][1]*m[0][1] +
            m[1][1]*m[1][1] +
            m[2][1]*m[2][1]);
        ...
        fInvLength = ss_fast_InvSqrt(m[0][2]*m[0][2] +//Math::InvSqrt(m[0][2]*m[0][2] +
            m[1][2]*m[1][2] +
            m[2][2]*m[2][2]);
        ...
        Vector3& kD, Vector3& kU) const
    {
        ...
        fInvLength = ss_fast_InvSqrt(kQ[0][1]*kQ[0][1] + kQ[1][1]*kQ[1][1] +//Math::InvSqrt(kQ[0][1]*kQ[0][1] + kQ[1][1]*kQ[1][1] +
            kQ[2][1]*kQ[2][1]);
        kQ[0][1] *= fInvLength;
        kQ[1][1] *= fInvLength;
        kQ[2][1] *= fInvLength;
        ...
        kQ[1][2] -= fDot*kQ[1][1];
        kQ[2][2] -= fDot*kQ[2][1];
        fInvLength = ss_fast_InvSqrt(kQ[0][2]*kQ[0][2] + kQ[1][2]*kQ[1][2] +//Math::InvSqrt(kQ[0][2]*kQ[0][2] + kQ[1][2]*kQ[1][2] +
            kQ[2][2]*kQ[2][2]);
        kQ[0][2] *= fInvLength;
        kQ[1][2] *= fInvLength;
        kQ[2][2] *= fInvLength;
        ...
        float& rfRAngle) const
    {
        ...
        rfPAngle = asin(m[0][2])/*Radian(Math::ASin(m[0][2]))*/;
        if ( rfPAngle < M_HALF_PI/*Radian(Math::HALF_PI)*/ )
        {
            if ( rfPAngle > -M_HALF_PI/*Radian(-Math::HALF_PI)*/ )
            {
                rfYAngle = atan2(-m[1][2],m[2][2])/*Math::ATan2(-m[1][2],m[2][2])*/;
                rfRAngle = atan2(-m[0][1],m[0][0])/* Math::ATan2(-m[0][1],m[0][0])*/;
                return true;
            }
            else
            {
                // WARNING.  Not a unique solution.
                float fRmY = atan2(m[1][0],m[1][1])/*Math::ATan2(m[1][0],m[1][1])*/;
        ...
        {
            // WARNING.  Not a unique solution.
            float fRpY = atan2(m[1][0],m[1][1])/*Math::ATan2(m[1][0],m[1][1])*/;
            rfRAngle = 0.0;  // any angle works
        ...
        float& rfRAngle) const
    {
        ...
        if ( rfPAngle < M_HALF_PI/*Radian(Math::HALF_PI)*/ )
        {
            if ( rfPAngle > -M_HALF_PI/*Radian(-Math::HALF_PI)*/ )
            {
                rfYAngle = atan2(m[2][1],m[1][1])/*Math::ATan2(m[2][1],m[1][1])*/;
                rfRAngle = atan2(m[0][2],m[0][0])/*Math::ATan2(m[0][2],m[0][0])*/;
                return true;
            }
            else
            {
                // WARNING.  Not a unique solution.
                float fRmY = atan2(-m[2][0],m[2][2])/* Math::ATan2(-m[2][0],m[2][2])*/;
                rfRAngle = 0.0;  // any angle works
                rfYAngle = rfRAngle - fRmY;
                return false;
            }
        ...
        {
            // WARNING.  Not a unique solution.
            float fRpY = atan2(-m[2][0],m[2][2])/*Math::ATan2(-m[2][0],m[2][2])*/;
            rfRAngle = 0.0;  // any angle works
            rfYAngle = fRpY - rfRAngle;
            return false;
        }
        ...
        float& rfRAngle) const
    {
        ...
        rfPAngle = asin(-m[1][2])/*Math::ASin(-m[1][2])*/;
        if ( rfPAngle < M_HALF_PI/*Radian(Math::HALF_PI)*/ )
        {
            if ( rfPAngle > -M_HALF_PI/*Radian(-Math::HALF_PI)*/ )
            {
