...
namespace HW
{
	...
	void Entity::getBoundingSphereImp()
	{
		if (!m_boundingSphereCurrent)
		{
			if (m_parent)
			{
				Vector3 center = m_Mesh->getModelBoundingSphere().getCenter();
				// transform, scaling is not supported.
				center = m_parent->getWorldMatrix().transformAffine(center);

				Vector3 scaled = m_parent->getScale();
				float max_scale = max(scaled.x, scaled.y);
				max_scale = max(scaled.z, max_scale);

				m_sphere.setRadius(m_Mesh->getModelBoundingSphere().getRadius()*max_scale);
	...
	{
		//TEST £º if the System is in Shadow Generation£¬test if this entity castShadow
