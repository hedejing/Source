...
namespace HW
{
	...
	void OctreeSceneManager::traverseOctree(Octree * tree, Camera * cam)
	{
		...
		bbox.setExtents(bbox.getMaximum() + half_size ,bbox.getMinimum() - half_size);
		unsigned int visibility = 0;
		if(visibility = cam->intersects(bbox))
		{
			bool test = false;
			bool insert = false;
		...
		/*else
		{
			...
			cam->intersects(bbox);
			Vector3 m_min = bbox.getMinimum(),m_max = bbox.getMaximum();
			...
			for(;it_node != nodelist.end(); it_node++)
			{
				m_min = it_node->second->getBoundingBox().getMinimum();
				m_max = it_node->second->getBoundingBox().getMaximum();
